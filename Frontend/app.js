...
//   } else {
//     // Update an existing expense in the array
...
async function fetchExpenses() {
    try {
        const response = await axios.get('http://localhost:8000/expense/get-expense'); // Replace with your API endpoint
...
async function saveOrUpdateExpense(index, amount, description, category) {
    try {
        if (isNaN(index)) {
            ...
            const response = await axios.post('http://localhost:8000/expense/add-expense', newExpense); // Replace with your API endpoint
            expenses.push(response.data);
        } else {
            // Update an existing expense
            const updatedExpense = { id: expenses[index].id, amount, description, category };
            await axios.put(`http://localhost:8000/expense/edit/${updatedExpense.id}`, updatedExpense); // Use the correct URL for editing
